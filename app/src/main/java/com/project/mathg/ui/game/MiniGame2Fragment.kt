...
import com.project.mathg.databinding.FragmentMiniGame2Binding
...
class MiniGame2Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        _binding = FragmentMiniGame2Binding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        timer = object : CountDownTimer(60000, 100) {
            ...
            override fun onFinish() {
                val highLevel = sharedPreferences.getInt("levelMiniGame2", 0)
                val highScore = sharedPreferences.getInt("scoreMiniGame2", 0)
                if (highLevel <= level) {
                    sharedPreferences.edit().putInt("levelMiniGame2", level).apply()
                }
                if (highScore <= score) {
                    sharedPreferences.edit().putInt("scoreMiniGame2", score).apply()
                }
                findNavController().navigate(R.id.action_MiniGame2Fragment_to_HomeFragment)
            }
        }
    ...
    private fun questions() {
        ...
        binding.imgFalse.setOnClickListener {
            if (rand12 != rand34) {
        ...
        binding.imgTrue.setOnClickListener {
            if (rand12 == rand34) {
                binding.tvResult.text = randCorrectAnswerDialog[Random.nextInt(0, 5)]
                binding.tvResult.setTextColor(Color.parseColor("#006400"))
                rSlider.slideEdge = Gravity.END
